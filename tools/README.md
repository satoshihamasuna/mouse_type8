...
通信時は走行パラメータを1000倍、細かなPIDゲインを保持するためPIDだけ100万倍の整数として返します。GUI上では元の単位へ自動変換されます。

`Degree correction` は経路生成で使う論理角度を変えず、実際の旋回プロファイルだけを補正します。たとえば右90°ターンで `Degree=-90`、`Degree correction=1.5` とすると、経路上は右90°のまま、実行プロファイルは-88.5°になります。
