...
#define FANMOT_DUTY_MIN	(20) // (DUTY_MIN)
#define FANMOT_DUTY_MAX	(980)	   // (980)
...
{
	uint32_t	pulse_l;
	uint32_t    pulse_min = (uint32_t)(((PCLK1) / PWMFREQ * MOT_DUTY_MIN/ 1000) - 1);
	// duty_l: -1000 ～ +1000
	...
	if( duty_l < 0 ) {
		...
		MOT_SET_COMPARE_L_REVERSE( TIM4->ARR - pulse_min );

	} else if( duty_l > 0 ) {
		MOT_SET_COMPARE_L_FORWARD( TIM4->ARR - pulse_min );
...
void Motor_SetDuty_Right( int16_t duty_r )
{
	...
	if( duty_r < 0 ) {
		...
		MOT_SET_COMPARE_R_REVERSE( TIM4->ARR - pulse_min );
	} else if( duty_r > 0 ) {
		MOT_SET_COMPARE_R_FORWARD( TIM4->ARR - pulse_min );
...
void FAN_Motor_SetDuty(int16_t duty_f)
{
	...
	uint32_t pulse_min = (uint32_t)((PCLK1) / FANPWMFREQ * FANMOT_DUTY_MIN/ 1000) - 1;

	if( ABS(duty_f) > FANMOT_DUTY_MAX ) {
		pulse_f = (uint32_t)((PCLK1) / FANPWMFREQ * FANMOT_DUTY_MAX / 1000) - 1;
	}else if(duty_f == 0){
		pulse_f = (uint32_t)((PCLK1) / FANPWMFREQ * FANMOT_DUTY_MIN/ 1000) - 1;
	}else if( ABS(duty_f) < FANMOT_DUTY_MIN ) {
		pulse_f = (uint32_t)((PCLK1) / FANPWMFREQ * FANMOT_DUTY_MIN/ 1000) - 1;
	} else {
	...
	//HAL_GPIO_WritePin(fn1_GPIO_Port, fn1_Pin, 0);
	__HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_4, TIM1->ARR - pulse_f);
	__HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_1, TIM1->ARR - pulse_min);
